...
name="qa-m4-token-$(date +%s)"
note "Creating token_bucket policy: ${name}"
create_body="{\"name\":\"${name}\",\"algorithm\":\"token_bucket\",\"params_json\":{\"capacity\":4,\"refill_rate_per_sec\":2,\"tokens_per_request\":2},\"enabled\":true,\"description\":\"qa smoke m4 token bucket\"}"
...
[ "$algorithm" = "token_bucket" ] || fail "created policy algorithm mismatch"
...
req_body='{"client_id":"qa-tb-client"}'
...
[ "$first_algo" = "token_bucket" ] || fail "simulate #1 algorithm mismatch"

for field in count window_start_ms window_size_sec state_schema_version tokens capacity refill_rate_per_sec tokens_per_request last_refill_ms; do
...
note "Sleeping 1.3s to allow refill"
sleep 1.3

note "Simulate request after refill (expect 200)"
after_result=$(request POST "${API_BASE}/simulate/request" "$req_body")
after_status=$(printf '%s' "$after_result" | sed -n '1p')
assert_http_status "$after_status" "200" "simulate request after refill"

note "M4 token_bucket smoke checks passed."
echo "[PASS] gateway M4 token_bucket smoke suite"
