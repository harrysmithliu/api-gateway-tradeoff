import type { SimulationStatus, TokenBucketSimulationConfig } from "../../types/tokenBucket";

type SimulationControlsProps = {
  config: TokenBucketSimulationConfig;
  status: SimulationStatus;
  onUpdateConfig: (next: Partial<TokenBucketSimulationConfig>) => void;
  onStart: () => void;
  onPause: () => void;
  onResume: () => void;
  onStop: () => void;
      ...
      <div className="field-grid six-columns">
        ...
        <label>
          ...
          <input
            ...
            min={0.01}
            step={0.1}
            value={config.refillRatePerSec}
            onChange={(event) => onUpdateConfig({ refillRatePerSec: Number(event.target.value) })}
          />
        </label>
        <label>
          Tokens / Request
          <input
            ...
            onChange={(event) => onUpdateConfig({ tokensPerRequest: Number(event.target.value) })}
          />
        </label>
        <label>
          RPS
