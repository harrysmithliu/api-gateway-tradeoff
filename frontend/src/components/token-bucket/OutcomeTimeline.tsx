...
export function OutcomeTimeline({ events }: OutcomeTimelineProps) {
  ...
  const chartData = useMemo(() => {
    ...
    decisionEvents.forEach((event) => {
      ...
      const point: TooltipPoint = {
        ...
        tokens: decision.algorithmState?.tokens ?? null,
        capacity: decision.algorithmState?.capacity ?? null,
        refillRatePerSec: decision.algorithmState?.refillRatePerSec ?? null,
        tokensPerRequest: decision.algorithmState?.tokensPerRequest ?? null,
      ...
      tooltip: {
        ...
        formatter: (params: { data?: [number, number, TooltipPoint] }) => {
          ...
          return [
            ...
            `tokens: ${point.tokens === null ? "-" : point.tokens.toFixed(3)}`,
  ...
  return (
    <section className="card panel">
      ...
      <p className="meta">Retry-after reflects how long until refill can satisfy the next request cost.</p>
      <div className="chart-surface" ref={containerRef} aria-label="Token bucket outcome timeline chart" />
    </section>
  );
}
