...
import { useSlidingWindowCounterSimulation } from "./useSlidingWindowCounterSimulation";

vi.mock("../api/slidingWindowCounterApi", () => ({
  ...
  fetchActiveSlidingWindowCounterPolicy: vi.fn(),
  simulateSlidingWindowCounterRequest: vi.fn(),
  syncSlidingWindowCounterPolicyConfig: vi.fn(),
}));
...
const mockedSimulate = vi.mocked(simulateSlidingWindowCounterRequest);
const mockedFetchActivePolicy = vi.mocked(fetchActiveSlidingWindowCounterPolicy);
const mockedSyncPolicy = vi.mocked(syncSlidingWindowCounterPolicyConfig);

describe("useSlidingWindowCounterSimulation", () => {
  beforeEach(() => {
    vi.useRealTimers();
    vi.clearAllMocks();

    mockedFetchActivePolicy.mockResolvedValue({
      id: "policy-sliding-window-counter",
      name: "sliding-window-counter-dashboard",
        ...
        event: {
          ...
          decision: {
            ...
            algorithmState: {
              ...
              previousWindowCount: count > 1 ? 2 : 0,
              previousWindowWeight: 0.3,
              estimatedCount: count + (count > 1 ? 0.6 : 0),
  ...
  it("dispatches requests and updates KPI snapshot consistently", async () => {
    const { result } = renderHook(() => useSlidingWindowCounterSimulation());
