...
export const useSlidingWindowCounterSimulation = (): UseSlidingWindowCounterSimulationValue => {
  ...
  const reloadActivePolicy = useCallback(async () => {
    ...
    try {
      const backendPolicy = await fetchActiveSlidingWindowCounterPolicy();
      ...
      if (backendPolicy) {
        ...
        setPolicySyncStatus("ready");
        setPolicySyncMessage("No active sliding window counter policy found. Start will create or reuse one.");
      }
    } catch (error) {
      if (!mountedRef.current) {
        return;
  ...
  const start = useCallback(() => {
    ...
    void (async () => {
      ...
      try {
        const syncedPolicy = await syncSlidingWindowCounterPolicyConfig({
