import type {
  SlidingLogAlgorithmState,
  SlidingLogApiResult,
  SlidingLogDecision,
  SlidingLogIssue,
  SlidingLogPolicySnapshot,
  SlidingLogSimulationPayload,
} from "../types/slidingLog";

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL ?? "http://localhost:8000";
const REQUEST_TIMEOUT_MS = 10_000;
const DASHBOARD_POLICY_NAME = "sliding-log-dashboard";
...
const createIssue = (issue: SlidingLogIssue): SlidingLogIssue => issue;
...
const mapAlgorithmState = (raw: unknown): { state: SlidingLogAlgorithmState | null; issues: SlidingLogIssue[] } => {
  if (!isRecord(raw)) {
    return {
      state: null,
      issues: [
  ...
  const oldestInWindowMs = toNumberOrNull(raw.oldest_in_window_ms);
...
): { decision: SlidingLogDecision | null; issues: SlidingLogIssue[] } => {
  if (!isRecord(raw)) {
    return {
      decision: null,
      issues: [
  ...
  if (algorithm !== "sliding_log") {
    issues.push(
      createIssue({
        ...
        severity: "warning",
        message: `Expected sliding_log but received ${algorithm}.`,
      }),
    );
  }

  const decision: SlidingLogDecision = {
...
const createSyntheticErrorResult = (message: string): SlidingLogApiResult => {
...
const mapPolicySnapshot = (raw: PolicyRaw): SlidingLogPolicySnapshot | null => {
  if (typeof raw.id !== "string" || typeof raw.name !== "string") {
    return null;
  }
  if (raw.algorithm !== "sliding_log") {
    return null;
  }
...
const listPolicies = async (): Promise<SlidingLogPolicySnapshot[]> => {
  const payload = await requestJson<PolicyRaw[]>("/api/policies");
  return payload.map(mapPolicySnapshot).filter((item): item is SlidingLogPolicySnapshot => item !== null);
};

const getActivePolicy = async (): Promise<SlidingLogPolicySnapshot | null> => {
  try {
...
const createPolicy = async (params: { name: string; limit: number; windowSizeSec: number }): Promise<SlidingLogPolicySnapshot> => {
  const payload = await requestJson<PolicyRaw>("/api/policies", "POST", {
    ...
    enabled: true,
    description: "Managed by sliding-log dashboard",
  });
...
const updatePolicy = async (policyId: string, params: { limit: number; windowSizeSec: number }): Promise<SlidingLogPolicySnapshot> => {
...
export const syncSlidingLogPolicyConfig = async (params: {
  limit: number;
  windowSizeSec: number;
  resetRuntimeState: boolean;
}): Promise<SlidingLogPolicySnapshot> => {
  const active = await getActivePolicy();

  let targetPolicy: SlidingLogPolicySnapshot;
...
export const fetchActiveSlidingLogPolicy = async (): Promise<SlidingLogPolicySnapshot | null> => getActivePolicy();

export const simulateSlidingLogRequest = async (
  payload: SlidingLogSimulationPayload,
): Promise<SlidingLogApiResult> => {
